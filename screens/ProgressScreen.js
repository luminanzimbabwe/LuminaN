...
const OrderProgressScreen = ({ navigation }) => {
  ...
  try {
    ...
    if (!response.ok) {
      console.error("Response not OK. Status:", response.status, "Body:", data);
    ...
    (data.orders || []).forEach((order) => {
      if (!orderAnimations[order._id]) {
        orderAnimations[order._id] = {
  ...
  setOrders(prevOrders => {
    ...
    if (index !== -1) {
      ...
      // If order not found, optionally add it
...
const handleDriverAction = async (orderId, action) => {
  if (!authToken) return;

  try {
    const res = await fetch(`${BACKEND_URL}/driver/orders/${orderId}/${action}/`, {
      method: "PATCH",
      headers: {
        "Content-Type": "application/json",
