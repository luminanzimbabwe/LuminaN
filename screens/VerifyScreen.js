...
const VerifyScreen = ({ navigation, route }) => {
  ...
  const { user, tokens, signIn, markVerified } = useAuth();

  // Optional: Get email/userId from route params if passed from registration
  ...
  const handleVerify = async () => {
    ...
    try {
      const response = await fetch(`${BACKEND_URL}/api/verify-code/`, {
        ...
        body: JSON.stringify({ code: verificationCode, email, user_id: userId }),
      });

      const data = await response.json();

      if (response.ok) {
        await markVerified(); // update verified status locally
        Alert.alert("Success", "Your account has been verified!");
        navigation.replace("MainApp"); // Navigate to main app/home screen
      } else {
  ...
  if (!email && !userId) {
    setErrorMessage("Cannot resend code: missing user information.");
    return;
  }
  ...
  try {
    const response = await fetch(`${BACKEND_URL}/api/resend-verification-code/`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
    ...
    <LinearGradient colors={["#0f2027", "#203a43", "#2c5364"]} style={styles.container}>
      <Animated.View style={[styles.card, { opacity: fadeAnim, transform: [{ translateY: slideAnim }] }]}>
        ...
        <Text style={styles.subtitle}>
          Enter the 6-digit code sent to your email or phone to activate your LuminaN account
        </Text>
